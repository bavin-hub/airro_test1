  ...
  return (
    <div className="chat-window flex flex-col flex-1 p-4">
      <div className="messages flex-grow mb-4 overflow-y-auto">
        {chat.length === 0 ? (
          <div className="text-center text-gray-900 font-bold">What can I help with? (Ai, Robots, R)</div>
        ) : (
          chat.map((msg, index) => (
              ...
              <div className='py-2 flex justify-end'>
                {msg.user && (
                  <div className="user-message bg-[#3C3D37] p-4 rounded-full inline-block max-w-full">
                    <strong className='text-amber-800'>User:</strong> {msg.user}
                  </div>
                )}
              </div>
              <div className='py-2'>
                {msg.bot && (
                  <div className="bot-message bg-[#3C3D37] p-4 rounded-full inline-block max-w-full">
                    <strong className='text-amber-800'>Bot:</strong> {msg.bot}
                  </div>
                )}
              </div>
            </div>
